...
Tile charts are great for ranking things over time or across categories, ex: causes of death by year, causes of morbidity by age group, etc. They're especially handy if you have a third, more broad category that you want to incorporate into your chart as you can use that category to determine color. 
...
We're going to use the `mtcars` data from the `datasets` package. (To see all data in the datasets package, run `library(help = "datasets")`.)
...
plot1 <- data1 %>%
 ...
 # scale_y_reverse(limits = c(12.5,0.5)) +
...
plot2 <- plot1 +
  ...
  # add labels 
  labs(title = "Monthly airline passenger numbers",
       subtitle = "1949-1960",
